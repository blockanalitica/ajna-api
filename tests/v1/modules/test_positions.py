...
def test_handle_debt_events(dummy_models):
    ...
    _handle_debt_events(models, wallet_address, 0, positions)
    ...
    # assert positions[pool_address_1]["debt"] == Decimal("0")  # TODO
    assert positions[pool_address_1]["t0debt"] == Decimal("225")
    ...
    # assert positions[0].debt == Decimal("0") # TODO
    assert positions[pool_address_2]["t0debt"] == Decimal("110")
...
def test_handle_quote_token_events(dummy_models):
    ...
    _handle_quote_token_events(models, wallet_address, 0, positions)
...
@pytest.mark.django_db
def test_save_wallet_positions(dummy_models):
    wallet_address, pool_address_1, pool_address_2 = dummy_models
    models = EthereumModels()

    save_wallet_positions(models, wallet_address, 0)

    positions = models.current_position.objects.all().order_by("block_number")
    ...
    # assert positions[0].debt == Decimal("0") # TODO
    assert positions[0].t0debt == Decimal("110")
    assert positions[0].t0debt == Decimal("110")
    assert positions[0].collateral == Decimal("80")
    assert positions[0].supply == Decimal("111")
    assert positions[0].block_number == 17732900
    assert positions[0].datetime == datetime(2023, 6, 11, 12, 42, 53)

    assert positions[1].wallet_address == wallet_address
    assert positions[1].pool_address == pool_address_1
    # assert positions[1].debt == Decimal("0")  # TODO
    assert positions[1].t0debt == Decimal("225")
    assert positions[1].collateral == Decimal("115")
