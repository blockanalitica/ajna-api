...
class Command(BaseCommand):
    def _fix_goerli(self):
        ...
        self.stdout.write("Fixing goerli pool snapshot actual_utilization")
        ...
        for dt, block in goerli_blocks.items():
            ...
            for pool in pools:
                ...
                if snapshot.actual_utilization != Decimal(pool["actualUtilization"]):
                    snapshot.actual_utilization = Decimal(pool["actualUtilization"])
                    snapshot.save(update_fields=["actual_utilization"])

    def _fix_ethereum(self):
        eth_blocks = self._get_eth_blocks()
        models = EthereumModels()

        self.stdout.write("Fixing ethereum pool snapshot actual_utilization")
