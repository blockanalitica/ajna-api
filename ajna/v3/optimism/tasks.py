...
from .chain import Optimism, OptimismModels
...
def fetch_market_price_task():
    models = OptimismModels()
    update_token_prices(models, network="optimism")
...
def save_network_stats_for_today_task():
    models = OptimismModels()
    dt = date.today()
    save_network_stats_for_date(models, dt, "optimism")
...
def save_network_stats_for_yesterday_task():
    ...
    models = OptimismModels()
    yesterday = (datetime.now() - timedelta(days=1)).date()
    save_network_stats_for_date(models, yesterday, "optimism")
