...
from .chain import Blast, BlastModels
...
def fetch_market_price_task():
    models = BlastModels()
    update_token_prices(models, network="blast")
...
def save_network_stats_for_today_task():
    models = BlastModels()
    dt = date.today()
    save_network_stats_for_date(models, dt, "blast")
...
def save_network_stats_for_yesterday_task():
    ...
    models = BlastModels()
    yesterday = (datetime.now() - timedelta(days=1)).date()
    save_network_stats_for_date(models, yesterday, "blast")
