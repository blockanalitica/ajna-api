...
from .chain import Rari, RariModels
...
def fetch_market_price_task():
    models = RariModels()
    update_token_prices(models, network="rari")
...
def save_network_stats_for_today_task():
    models = RariModels()
    dt = date.today()
    save_network_stats_for_date(models, dt, "rari")
...
def save_network_stats_for_yesterday_task():
    ...
    models = RariModels()
    yesterday = (datetime.now() - timedelta(days=1)).date()
    save_network_stats_for_date(models, yesterday, "rari")
