...
from .chain import Filecoin, FilecoinModels
...
def fetch_market_price_task():
    models = FilecoinModels()
    update_token_prices(models, network="filecoin")
...
def save_network_stats_for_today_task():
    models = FilecoinModels()
    dt = date.today()
    save_network_stats_for_date(models, dt, "filecoin")
...
def save_network_stats_for_yesterday_task():
    ...
    models = FilecoinModels()
    yesterday = (datetime.now() - timedelta(days=1)).date()
    save_network_stats_for_date(models, yesterday, "filecoin")
