...
from .chain import Hemi, HemiModels
...
def fetch_market_price_task():
    models = HemiModels()
    update_token_prices(models, network="hemi")
...
def save_network_stats_for_today_task():
    models = HemiModels()
    dt = date.today()
    save_network_stats_for_date(models, dt, "hemi")
...
def save_network_stats_for_yesterday_task():
    ...
    models = HemiModels()
    yesterday = (datetime.now() - timedelta(days=1)).date()
    save_network_stats_for_date(models, yesterday, "hemi")
