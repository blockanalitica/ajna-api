...
from .chain import Linea, LineaModels
...
def fetch_market_price_task():
    models = LineaModels()
    update_token_prices(models, network="linea")
...
def save_network_stats_for_today_task():
    models = LineaModels()
    dt = date.today()
    save_network_stats_for_date(models, dt, "linea")
...
def save_network_stats_for_yesterday_task():
    ...
    models = LineaModels()
    yesterday = (datetime.now() - timedelta(days=1)).date()
    save_network_stats_for_date(models, yesterday, "linea")
