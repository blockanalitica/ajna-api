...
from .chain import Avalanche, AvalancheModels
...
def fetch_market_price_task():
    models = AvalancheModels()
    update_token_prices(models, network="avalanche")
...
def save_network_stats_for_today_task():
    models = AvalancheModels()
    dt = date.today()
    save_network_stats_for_date(models, dt, "avalanche")
...
def save_network_stats_for_yesterday_task():
    ...
    models = AvalancheModels()
    yesterday = (datetime.now() - timedelta(days=1)).date()
    save_network_stats_for_date(models, yesterday, "avalanche")
